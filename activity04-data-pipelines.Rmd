...
**Response**: When producing the document it shows the code messages produced. For instance the `load_packages` code chunk produces a message
showing the attached packages and any conflicts with other packages. This particular message shows conflicts with `filter` and `lag`
functions. This is to be expected when loading `{tidyverse}` because these functions already exist in base R.
...
**Response**: The `message` option in `load_data` shows the details in the assignment code line. It states the data is being seperated by
...
**Response**: We are going to need to compare the majors by `unemployment rate`. In a program like Excel this would be done with a sort feature (low to high).
...
```{r lowest_unemployment, message = FALSE}
college_recent_grads %>%
  arrange(unemployment_rate) %>%
  select(rank, major, unemployment_rate)
```

**Response**: This is a pretty nifty tool, we can see that the top 5 results all have 0% unemployment rate! Shout out to Math and Botany :).,
...
**Response**: By observing the data arranged in descending order of `sharewomen` we see that the major with the largest proportion of women is Early Childhood Education at nearly 97%.
...
**Response**: We can see that there are 10 STEM majors that are below the median salary of all majors. Lowest being Zoology.
