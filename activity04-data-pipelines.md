...
1.  It has been named (i.e., `load_packages`). Notice that the name
    ...
    view. Another way to see this information by clicking on the “# Data
    and packages” at the bottom left-hand area of this `.Rmd` pane.
...
**Response**: When producing the document it shows the code messages
produced. For instance the `load_packages` code chunk produces a message
showing the attached packages and any conflicts with other packages.
This particular message shows conflicts with `filter` and `lag`
functions. This is to be expected when loading `{tidyverse}` because
these functions already exist in base R.
...
**Response**: The `message` option in `load_data` shows the details in
the assignment code line. It states the data is being seperated by
commas and the columns and what type of data they’re being saved as.
...
**Response**: We are going to need to compare the majors by
`unemployment rate`. In a program like Excel this would be done with a
...
**Response**: This is a pretty nifty tool, we can see that the top 5
results all have 0% unemployment rate! Shout out to Math and Botany :).,
...
**Response**: By observing the data arranged in descending order of
`sharewomen` we see that the major with the largest proportion of women
is Early Childhood Education at nearly 97%.
...
**Response**: We can see that there are 10 STEM majors that are below
the median salary of all majors. Lowest being Zoology.
